...
describe("GetMarketAnalysisUseCase", () => {
  it("orchestrates candles, moving averages, and summary", async () => {
    ...
    const result = await useCase.execute("BTCUSDT", "1h", 230, [20, 50, 200]);
    ...
    expect(result.movingAverages.lines).toEqual([
      expect.objectContaining({ period: 20, values: expect.any(Array) }),
      expect.objectContaining({ period: 50, values: expect.any(Array) }),
      expect.objectContaining({ period: 200, values: expect.any(Array) })
    ]);
    expect(result.movingAverages.lines[1]?.values.at(-1)).toBe(205.5);
