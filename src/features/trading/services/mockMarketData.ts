...
export function buildMockMarketAnalysis(asset: CryptoAsset, timeframe: Timeframe, limit = 300): MarketAnalysisViewModel {
  const candles = generateMockCandles(asset, timeframe, limit);
  const smaLines = buildSmaLines(candles);
...
function buildSmaLines(candles: readonly Candle[]): SmaLines {
  return {
    sma20: calculateSmaPoints(candles, 20),
    sma93: calculateSmaPoints(candles, 93),
    sma230: calculateSmaPoints(candles, 230)
  };
}
